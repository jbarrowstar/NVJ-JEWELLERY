...
export default function Sidebar({ isOpen, toggleSidebar }: SidebarProps) {
  ...
  return (
    <div
      className={`fixed top-16 left-0 h-[calc(100vh-4rem)] bg-white shadow-md flex flex-col justify-between transition-all duration-300 z-40 ${
        isOpen ? 'w-64' : 'w-16'
      }`}
    >
      {/* 🔘 Sidebar Toggle */}
      ...
      {/* 🏷️ Footer Branding */}
      <div className="text-gray-500 text-xs p-4 border-t border-gray-200 text-center leading-snug">
        {isOpen ? (
          <>
            <p className="mt-1">Application Created and Service provided by
              <br></br> 
            <span className="font-semibold text-gray-700">JB Arrowstar Solutions Pvt Ltd</span></p>
          </>
        ) : (
          <p className="origin-bottom-left whitespace-nowrap text-[10px] text-gray-900">
            JBAS
          </p>
        )}
      </div>
